# OpenAPI diff — v1.0.0 → v1.1.0

Generated with [oasdiff](https://github.com/oasdiff/oasdiff):

```
$ oasdiff breaking openapi/openapi-v1.0.0.json openapi/openapi-v1.1.0.json --fail-on ERR
No breaking changes to report, but the specs are different.
Run 'oasdiff diff' to see structural differences.

$ oasdiff changelog openapi/openapi-v1.0.0.json openapi/openapi-v1.1.0.json
3 changes: 0 error, 0 warning, 3 info
info	[endpoint-added] at /specs/openapi-v1.1.0.json
...
`1.0.0 → 1.1.0`. The feature shipped in Phase 4 (canary at
`docs/canary-report.md`, then tagged `v1.1.0`). Phase 4's own changes — the
moderation-notification Celery task and its `notifications` table — add no HTTP
surface, so the spec is unchanged between `v1.1.0-rc1` and `v1.1.0`.

The check is enforced in CI (`contract-test.yml`): the job fails if
`oasdiff breaking` finds anything between the last released spec
(`openapi-v1.0.0.json`) and `HEAD` (`openapi.json`).
